...
def reconcile_timestamp(timestamp, scene_changes, silence_periods,
    ...
    direction="start" -> for clip STARTS: prefer silence.END (speech resumes here).
                         Never snap to silence.start or midpoint -- those are inside silence.
    direction="end"   -> for clip ENDS:   prefer silence.START (speech ends here).
                         Never snap to silence.end or midpoint.
    direction="any"   -> for non-clip timestamps: use nearest silence boundary (no midpoint).

    Priority: audio silence boundary > scene change > original.
    Returns (snapped_timestamp, reason_string).
    """
    # -- Direction-aware silence candidate selection --
    # NEVER use midpoint -- it lands inside silence causing audio gaps
    if direction == "start":
        # For clip starts: only silence.END is valid (speech resumes = clean entry)
        silence_candidates = [(p["end"], p) for p in silence_periods]
    elif direction == "end":
        # For clip ends: only silence.START is valid (speech stops = clean exit)
        silence_candidates = [(p["start"], p) for p in silence_periods]
    else:
        # For point timestamps: nearest of start or end (not midpoint)
        silence_candidates = []
...
def analyze_video_structure(video_path):
    """Run scene + silence detection. Returns dict with results."""
    logger.info(f"[frame_analyzer] Analyzing: {Path(video_path).name}")
    scene_changes   = detect_scene_changes(video_path, threshold=0.30)
    silence_periods = detect_silence_periods(video_path, noise_db=-28.0, min_duration=0.08)
...
def reconcile_analysis_timestamps(analysis, video_structure):
    ...
    Uses direction-aware snapping: starts -> silence.end, ends -> silence.start.
    Returns updated analysis (original NOT mutated).
    """
    ...
    # Segment boundaries -- direction-aware
    ...
    # Hook best_clips -- direction-aware (MOST IMPORTANT: start -> silence.end)
    ...
    # Shot groups with start/end -- direction-aware
    ...
    # Action cut risks (point timestamps)
